...
def _construir_dia(nombre_dia, enfoque, spec, nivel, peso_usuario, usados_global):
    ...
    return {"dia": nombre_dia, "enfoque": enfoque, "ejercicios": ejercicios}, usados_global
...
# ── EMPUJE: siempre 3+3+2, sin excepciones por prioritarios ──
def _spec_empuje(prioritarios):
    return [
        ("pecho",           3),
        ("deltoides_medio", 3),
        ("triceps",         2),   # ← fijo siempre en 2
    ]


# ── TIRÓN: siempre 3+2+2, sin excepciones por prioritarios ──
def _spec_tiron(prioritarios):
    return [
        ("espalda",             3),
        ("deltoides_posterior", 2),
        ("biceps",              2),   # ← fijo siempre en 2
    ]
